        ...
        with self.__mutex:
            ...
            if len(self.__client_queue) and not self.__last_msg and elapsed > 5:
                ...
                if byte8 >= 16:
                    ...
                    if byte8 & 16:
                        cmd.extend(["--controller", os.environ['DERP_ROOT'] + "/model/square"])
                    elif byte8 & 32:
                        cmd.extend(["--controller", os.environ['DERP_ROOT'] + "/model/cross"])
                    elif byte8 & 64:
                        cmd.extend(["--controller", os.environ['DERP_ROOT'] + "/model/circle"])
                    elif byte8 & 128:
                        cmd.extend(["--controller", os.environ['DERP_ROOT'] + "/model/triangle"])
                    print(cmd)
                    Popen(cmd)
        return True
