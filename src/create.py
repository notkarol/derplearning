...
def process_recording(sw_config, exp, recording_path,
    ...
    inferer = derp.inferer(hw_config, sw_config, exp)
    ...
    while video_cap.isOpened() and frame_id < len(label_timestamps):
        ...
        for pos, pd in enumerate(sw_config[exp]['predict']):
            ...
            state_pos = state_headers.index(name)
            timestamp = state_timestamps[frame_id] + int(pd['delay'] * 1E6)
            state[pos] = util.find_value(state_timestamps, timestamp,
                                         states[:, state_pos])
...
def main(args):
    ...
    if getsize(train_states_path) == 0:
        headers = (['key'] +
                   ...
                   [key for key in sw_config[args.name]['perturbations']])
    ...
    for data_folder in sw_config[args.name]['data_folders']:
        ...
        for filename in os.listdir(data_folder):
            ...
            if isdir(recording_path):
                process_recording(sw_config, args.name, recording_path,
                                  train_states_fd, val_states_fd,
                                  train_dir, val_dir)
