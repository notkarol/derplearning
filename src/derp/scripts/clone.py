...
class Clone(Inferer):
    ...
    def __init__(self, hw_config, sw_config, model_dir, state, nocuda=False):
    ...
    def plan(self, state):
        ...
        if state['record'] and self.sw_config['debug']:
            util.save_image(join(state['folder'], "%i_frame.png" % state['timestamp']), frame)
            util.save_image(join(state['folder'], "%i_patch.png" % state['timestamp']), patch)
            util.save_image(join(state['folder'], "%i_thumb.png" % state['timestamp']), thumb)
