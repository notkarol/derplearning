...
class Resnet33Model(nn.Module):
    def __init__(self, in_dim, n_status, n_out, verbose=True):
        ...
        self.c2a = ResnetBlock(dim, 64, verbose=verbose)
        self.c2b = ResnetBlock(dim, 64, verbose=verbose)
        self.c2c = ResnetBlock(dim, 64, pool='max', verbose=verbose)
        self.c3a = ResnetBlock(dim, 64, verbose=verbose)
        self.c3b = ResnetBlock(dim, 64, verbose=verbose)
        self.c3c = ResnetBlock(dim, 64, verbose=verbose)
        self.c3d = ResnetBlock(dim, 64, verbose=verbose)
        self.c3e = ResnetBlock(dim, 64, verbose=verbose)
        self.c3f = ResnetBlock(dim, 64, pool='max', verbose=verbose)
        ...
        self.c4f = ResnetBlock(dim, 64, pool='max', verbose=verbose)
