...
class Resnet21Model(nn.Module):
    def __init__(self, in_dim, out_dim, verbose=True):
        super(Resnet21Model, self).__init__()
        dim = in_dim.copy()
        ...
        self.c3a = ResnetBlock(dim, 40, verbose=verbose)
        self.c3b = ResnetBlock(dim, 40, verbose=verbose)
        self.c3c = ResnetBlock(dim, 40, pool='max', verbose=verbose)
        self.c4a = ResnetBlock(dim, 48, verbose=verbose)
        self.c4b = ResnetBlock(dim, 48, verbose=verbose)
        self.c4c = ResnetBlock(dim, 48, pool='max', verbose=verbose)
