...
class Servo:
    ...
    def __init__(self, config):
        ...
        self.speed_offset = 0
        self.steer_offset = 0
        self.__context, self.__subscriber = derp.util.subscriber(['/tmp/derp_brain',
                                                                  '/tmp/derp_joystick',
                                                                  '/tmp/derp_keyboard'])
        self.__connect()
    ...
    def run(self):
        ...
        if topic == 'control':
            ...
            if self.auto or message.manual:
                self.__send(message.speed + self.speed_offset, self.config['speed_index'],
                            self.config['speed_min'], self.config['speed_max'])
                self.__send(message.steer + self.steer_offset, self.config['steer_index'],
                            self.config['steer_min'], self.config['steer_max'])
